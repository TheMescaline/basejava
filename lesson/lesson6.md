...
- Рефакторинг: в конструктор ru.javawebinar.model.Resume добавить второй параметр `fullName`
- Сделать рефакторинг всех реализаций `Storage`: заменить метод `ru.javawebinar.model.Resume[] getAll()` на `List<ru.javawebinar.model.Resume> getAllSorted()`
